...
// TestBuildListDirQuery verifies the SQL generated by buildListDirQuery.
//
// Key invariants:
//  1. The ORDER BY must use a CASE expression to put folders first, not the
//     raw f.type column.  The old form "ORDER BY f.type ASC" grouped images
//     before videos lexicographically, so videos never appeared in sort order
//     when mixed with images.
//  2. The filter clause is added only when withFilter=true.
//  3. The caller-supplied sort expression and direction appear verbatim after
//     the CASE expression.
func TestBuildListDirQuery(t *testing.T) {
	t.Parallel()
	...
	const (
		wantCaseExpr = "CASE WHEN f.type = 'folder' THEN 0 ELSE 1 END ASC"
		oldBuggyExpr = "f.type ASC"
		filterClause = "AND (f.type = 'folder' OR f.type = ?)"
	)

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			q := buildListDirQuery(tt.orderExpr, tt.sortDir, tt.withFilter)

			// Must use the CASE expression for the primary sort key.
			if !strings.Contains(q, wantCaseExpr) {
				t.Errorf("ORDER BY should contain CASE expression %q\nGot: %s", wantCaseExpr, q)
			}

			// Must NOT use f.type ASC as the primary ORDER BY key (the regression).
			// Note: when the user sorts by type, f.type ASC is legitimately the
			// secondary key after the CASE expression, so we only check for primary.
			if strings.Contains(q, "ORDER BY "+oldBuggyExpr) {
				t.Errorf("ORDER BY must not start with %q (old bug: groups all images before all videos)\nGot: %s", oldBuggyExpr, q)
			}

			// The caller-supplied sort expression must appear after the CASE,
			// within the ORDER BY clause.  Search from the ORDER BY keyword to
			// avoid matching column names in the SELECT list.
			orderByMarker := "ORDER BY"
			orderByPos := strings.Index(q, orderByMarker)
			if orderByPos < 0 {
				t.Fatalf("ORDER BY keyword missing from query\nGot: %s", q)
			}
			orderByClause := q[orderByPos:]
			casePos := strings.Index(orderByClause, wantCaseExpr)
			exprPos := strings.Index(orderByClause, tt.orderExpr)
			if casePos < 0 || exprPos <= casePos {
				t.Errorf("sort expression %q must appear after CASE expression in ORDER BY\nGot: %s", tt.orderExpr, q)
			}

			// The sort direction must appear in the query.
			if !strings.Contains(q, tt.sortDir) {
				t.Errorf("query should contain sort direction %q\nGot: %s", tt.sortDir, q)
			}

			// Filter clause present iff requested.
			hasFilter := strings.Contains(q, filterClause)
			if tt.withFilter && !hasFilter {
				t.Errorf("withFilter=true but filter clause %q missing\nGot: %s", filterClause, q)
			}
			if !tt.withFilter && hasFilter {
				t.Errorf("withFilter=false but filter clause %q present\nGot: %s", filterClause, q)
			}
		})
	}
}
