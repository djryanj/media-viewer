...
- perf(frontend): every tag add or remove in the Tags modal triggered a redundant second `GET /api/tags/file` request. `addTag` and `removeTag` called `loadFileTags()` (which populates `currentTagsList`) and then immediately called `refreshGalleryItemTags()` which fetched the same data again to update the gallery card. Both now call `updateGalleryItemTagsDOM(path, currentTagsList)` directly, cutting per-operation API requests from 2 → 1. [#399](https://github.com/djryanj/media-viewer/issues/399)
- perf(frontend): bulk tag operations (`addBulkTag`, `removeBulkTag`, `mergeTagToAll`) fired two sequential `POST /api/tags/query` requests per operation — one from `loadBulkTags()` to populate the modal, and a second from `batchRefreshGalleryItemTags()` to update the gallery cards. `loadBulkTags()` now returns its `tagsByPath` map; `batchRefreshGalleryItemTags()` accepts an optional `prefetchedTagsByPath` parameter and, when supplied, skips the API call entirely. All three callers pass the returned map through, cutting bulk-operation API traffic from 3 → 2 requests per mutation. [#399](https://github.com/djryanj/media-viewer/issues/399)
- perf(frontend): `openTagsDrawer()` in `lightbox.js` called `lucide.createIcons({ nodes: [this.elements.tagsDrawer] })` on every drawer open. The drawer's static icons (title, close, copy, paste, add buttons) are already converted to SVG once by `lucide.createIcons()` at the end of `createTagsDrawer()`. Tag chips rendered by `renderDrawerTags()` use inline SVG, not `data-lucide` placeholders, so there is never anything to re-initialize on subsequent opens. The redundant call is removed. [#399](https://github.com/djryanj/media-viewer/issues/399)
- perf(frontend): `removeTagFromDrawer()` in `lightbox.js` called `Tags.refreshGalleryItemTags(path)` (a `GET /api/tags/file` round-trip) and `Tags.loadAllTags()` (a `GET /api/tags` round-trip) after every drawer remove, even though `file.tags` was already updated in-place by the optimistic filter above. The gallery card now updates via `Tags.updateGalleryItemTagsDOM(path, file.tags)` directly. `loadAllTags()` is also removed from the remove path since removing a file-tag association never introduces new tags into the catalog. [#399](https://github.com/djryanj/media-viewer/issues/399)
- perf(frontend): `addTagFromDrawer()` in `lightbox.js` called `Tags.refreshGalleryItemTags(path)` after a successful add, making a redundant `GET /api/tags/file` request when `file.tags` was already updated optimistically. The gallery card now updates via `Tags.updateGalleryItemTagsDOM(path, file.tags)` directly. The drawer's `allTagSuggestions` cache is also cleared (`this.allTagSuggestions = []`) so that a newly-added tag appears in autocomplete on the next drawer open, fixing a latent bug where the suggestion list would never reflect tags added mid-session. [#399](https://github.com/djryanj/media-viewer/issues/399)
