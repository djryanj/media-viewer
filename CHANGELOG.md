...
- build(deps): bump golangci/golangci-lint-action from 9.2.0 to 9.2.1 ([#557](https://github.com/djryanj/media-viewer/pull/557))
...
- fix(frontend): Opening media from the main search results view now keeps lightbox browsing scoped to the current search result set instead of jumping back to the folder's normal file order. ([#528](https://github.com/djryanj/media-viewer/issues/528))
- fix(backend): Session cookies now work correctly in WebKit and mobile Safari when the server is accessed over plain HTTP (local and development deployments). Previously, the `Secure` attribute was set unconditionally on all session cookies; WebKit strictly enforces the RFC 6265 rule that a `Secure` cookie received over HTTP must not be sent back on HTTP, so all API calls after the initial login silently failed with 401s. The `Secure` attribute is now only set when the connection is actually over TLS, matching the behaviour of other browsers and matching the production deployment model. ([#528](https://github.com/djryanj/media-viewer/issues/528))
...
- fix(backend): A number of latent lint warnings silently introduced by golangci-lint v2.12.0 (bumped by [#519](https://github.com/djryanj/media-viewer/pull/519)) are now resolved. ([#531](https://github.com/djryanj/media-viewer/pull/531))
