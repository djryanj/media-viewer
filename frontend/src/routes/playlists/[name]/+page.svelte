<script lang="ts">
    ...
    function handleKeydown(e: KeyboardEvent) {
        ...
        else if (e.key === 'Escape') { if (sidebarOpen) sidebarOpen = false; else goto('/'); }
...
<svelte:window onkeydown={handleKeydown} ontouchstart={handleTouchStart} ontouchend={handleTouchEnd} />
    ...
    <div class="player-body">
        ...
        <aside class="sidebar" class:open={sidebarOpen} aria-label="Playlist items">
            <div class="sidebar-header">
                ...
                <button class="sidebar-close" onclick={() => (sidebarOpen = false)} aria-label="Close playlist">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
            ...
            <ul class="item-list" role="listbox" aria-label="Playlist items">
                {#each playlist?.items ?? [] as item, i (i)}
                    <li
                        ...
                        onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') selectItem(i); }}
                    >
                        ...
                        {#if i === currentIndex}
                            <svg class="item-playing-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
<style>
    ...
    .header-back svg, .header-btn svg { width: 20px; height: 20px; }
    ...
    .empty button:hover,
    .missing button:hover { background: rgba(255,255,255,0.2); }
    ...
    .item.active { background: rgba(200,255,0,0.06); }
    .item.missing { opacity: 0.4; cursor: default; }
    ...
    .item.active .item-index { color: var(--color-primary, #c8ff00); }
    ...
    .item.active .item-name { color: #fff; font-weight: 500; }
